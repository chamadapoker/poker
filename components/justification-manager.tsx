...
// import { militaryPersonnel } from "@/lib/static-data" // Removido
...
export function JustificationManager() {
  ...
  const fetchMilitary = async () => {
    try {
      const { data, error } = await (supabase as any)
        ...
        .select('*')
        .order('name', { ascending: true }) // Ordenação básica por nome

      if (error) throw error
      console.log('Efetivo carregado:', data)
  ...
  const getSelectedMilitaryDisplay = () => {
    const military = militaryList.find(m => m.id === formData.military_id)
    return military ? `${military.rank} ${military.name}` : ""
  }

  const getMilitaryDisplayName = (militaryId: string) => {
    // Tenta achar na lista dinâmica, se não achar (ex: excluido), retorna o nome salvo na justificativa se tiver, ou id
    const military = militaryList.find(m => m.id === militaryId)
    if (military) return `${military.rank} ${military.name}`

    // Fallback: Tentar achar nas justificativas se tem o nome salvo
    const just = justifications.find(j => j.military_id === militaryId)
    return just?.military_name || "Militar não listado"
  }

-- =====================================================
-- SCRIPT BÁSICO - APENAS USUÁRIOS (SEM PROFILES)
-- Sistema POKER 360 - 1º/10º GAV
-- =====================================================

-- 1. Criar função simples para verificar se o usuário é admin
-- (Assumindo que todos os usuários logados são admin por enquanto)
...
-- 2. Criar view básica para listar usuários (apenas dados do auth.users)
...
-- 5. Comentários
COMMENT ON FUNCTION is_admin() IS 'Verifica se o usuário atual é administrador (básico)';
COMMENT ON FUNCTION get_user_stats() IS 'Obtém estatísticas básicas de usuários (apenas para admins)';
COMMENT ON VIEW admin_users_view IS 'View básica para listar usuários (apenas para admins)';

-- 6. Teste básico
DO $$
BEGIN
    RAISE NOTICE 'Configuração básica criada com sucesso!';
    ...
    RAISE NOTICE 'NOTA: Todos os usuários logados são considerados admin por enquanto';
END $$;
